...
- 房间色：projects=石墨 writing=鼠尾草 music-wall=雾蓝 my-music=深雾蓝 photography=银盐紫 knowledge=夜蓝 life=干玫瑰 menu=赭黄 mystery=焦赭 social=紫灰 special=奖章卡其金 about=干玫瑰（2026-06-17 由黄铜改）；组件内用 `var(--room)`（由页面注入）。⚠️ 改/加房间色要动**两处**：tokens.css 的 --paint-* 与 `src/lib/cover.ts` 的 ROOM_BASE/NEIGHBORS（门厅生成式封面的独立色表，漏了会静默落兜底黄铜褐 → 2026-07-18 两厅撞色事故）
