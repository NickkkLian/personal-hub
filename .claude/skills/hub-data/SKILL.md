...
| ~~永不进同步清单的 app~~          | —                                       | —                                          | **名单见下方「隐私铁律」，别在此复制**          |

**双仓库权限隔离架构（2026-06-12 定稿）**
> 📍 仓库清单 / 发布流 / 各 app 数据文件的**权威在 `HUB-APPS.md` §1 与 §4**；此处只留数据层实现要看的要点。两边不一致时以 HUB-APPS 为准。
> 另注：下面「公开文件清单」是**可能产出的全集**，不代表已发布——2026-07-14 实测 Database-Public 里只有 `album-journal.public.json` + `develop.public.json` 两个。
...
> ⚠️ **「哪些 app 永不公开」的权威名单在 `HUB-APPS.md` §2**（现 5 个：Investment-Info / Job-Tracker / People-Atlas / Media-Ops / Toronto-Plan）。
> 本文件此前自抄过一份、**漏了 People-Atlas / Media-Ops / Toronto-Plan**——而这份技能恰恰是改 sync 清单时自动加载的，最危险。2026-07-14 已改为指针：**动同步清单前先去 §2 核对，别信任何别处的副本。**
> 下面 4 条是**同步脚本自身的行为规则**（本文件独有，不与 HUB-APPS 重复）：
